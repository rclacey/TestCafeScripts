...
fixture `newsroom.photography.buildings`
    .page `https://newsroom.clevelandclinic.org/photography-2/`;

test('buildings', async t => {
    await t
        .click(Selector('.gallery-icon.landscape').nth(22).find('.attachment-thumbnail.size-thumbnail'))
...
test('regional_hospitals', async t => {
    await t
...
test('national_international_locations', async t => {
    await t
...
test('family_health_centers', async t => {
    await t
...
test('cleveland_clinic_history', async t => {
    await t
